...
class potrazivanjanizr extends stranica{
	function Prikaz(){
...
if ($pass != $info['lozinka']) 
	...
	{
		$status = $info['status'];
	if($status < 2){echo "Da bi mogli ispisati dugovanja u nizu morate imati odobrenje od strane administratora.<br> (Administrator vam mora promijeniti status.)!<br><br>";
...
echo "<br>Ispis potraživanja u nizu ($brojniza):<br><br>";  

$name="select * from potrazivanja where firma_potrazuje  = $fid order by datum desc";
...
for ($i=1; $i<=$brojniza; $i++){
	...
	if($red=mysql_fetch_array($du)){
		...
		$fideli=$firma_ime;
		$dug="select * from potrazivanja where firma_potrazuje = $fideli order by datum desc";
	if(!$du=mysql_query($dug)){echo "<br>Nastala je greška pri izvođenju upita!<br>".mysql_query();}
	if($red=mysql_fetch_array($du)){
		$d=$red["datum"];
		$firma_ime=$red["firma_duguje"];
	   ...
	   $novi_fid=$firma_ime;
		...
		if($i > 1){
		if ($novi_fid == $fideli){
		//echo "Firma $r nema potraživanja!<br><br>";
		break;}
...
<?
			...
			if($fd[$j] > $osam){
                  ...
                  <td align="left" class="style50"><?=$f[$j]?></td>
                  <td align="center" width="200" class="style50"><?=$fd[$j]?></td>
                  </tr>
                   <?
					}
			elseif ($fd[$j] < $osam and $fd[$j] > $petna){
				    ?>
                  <tr bgcolor="#006600">
                  <td width="20" align="center"><?=$c?></td>
                  <td align="left" class="style50"><?=$f[$j]?></td>
		...
		elseif ($fd[$j] < $petna and $fd[$j] > $dvad){
			  ?>
                  <tr bgcolor="#FF3300">
                  <td width="20" align="center"><?=$c?></td>
                  <td align="left" class="style50"><?=$f[$j]?></td>
			...
			elseif ($fd[$j] < $dvad and $fd[$j] > $trid){
				   ?>
                  <tr bgcolor="#663300">
                  <td width="20" align="center"><?=$c?></td>
                  <td align="left" class="style50"><?=$f[$j]?></td>
            ...
            <tr bgcolor="#CCCCCC">
            <td width="20" align="center"><?=$c?></td>
            <td align="left"><?=$f[$j]?></td>
            <td align="center" width="200"><?=$fd[$j]?></td>
  	        </tr>
            <?
					}
					?> 
...
$potrazivanjanizr = new potrazivanjanizr();
$potrazivanjanizr -> Prikaz();
ob_flush();
?>