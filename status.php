...
function PrikazSadrzaj(){
    ...
    if(!$_POST["status"])
			{
            ?>
<form action="" method="post">Promjena razine prava korisnika: <br /><br />
<select name="razina">
<option>0</option>
<option>1</option>
<option>2</option>
   ...
   <input type="submit" name="status" value="Promijeni" />
</form>
<?			}else{
		
			$raz=$_POST["razina"];
			$sql="update korisnik set status = '$raz' where korisnik_id = '$koris'";
			if (mysql_query($sql))
			{
			if (mysql_affected_rows()>0)
				{
				echo "<br>Podaci su uspješno izmjenjeni!<br>";
				?><a href="status.php?kor=<?=$koris?>"><br />Povratak</a><?
